...
class RegistrationDatabase:
    ...
    def _ensure_tables(self):
        ...
        for table, column, column_type in [
            ("pending_requests", "discord_username", "TEXT"),
            ("pending_requests", "telegram_username", "TEXT"),
            ("pending_requests", "updated_at", "TIMESTAMP"),
            ("affiliates", "discord_user_id", "TEXT"),
            ("affiliates", "telegram_links_sent", "TEXT"),
        ]:
            try:
    ...
    def add_registration(
        ...
        bypass_duplicate_checks = should_bypass_duplicate_checks(name)
        ...
        try:
            if not bypass_duplicate_checks:
                ...
                # Check if this Discord user is already pending/verified.
                cursor.execute(
                    "SELECT 1 FROM pending_requests WHERE discord_user_id = ? AND status = 'pending'",
                    (str(discord_user_id),)
                )
                if cursor.fetchone():
                    return False, "This Discord user already has a pending registration!"
                ...
                if cursor.fetchone():
                    return False, "This Discord user is already verified!"

                # Name-based matching is the MVP, so block duplicate name+country.
                cursor.execute(
                    "SELECT name, country FROM pending_requests WHERE status = 'pending'"
                )
                for existing_name, existing_country in cursor.fetchall():
                    if (
                        normalize_name(existing_name) == normalized_name
                        and normalize_name(existing_country) == normalized_country
                    ):
                        return False, "This name and country already has a pending registration!"
                ...
                for existing_name, existing_country in cursor.fetchall():
                    if (
                        ...
                        return False, "This name and country is already verified!"
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, 'discord', 'pending', CURRENT_TIMESTAMP)
