...
async def scrape_trade_nation_registrations(search_terms: list[str] | None = None) -> list[dict]:
    """
    ...
    async def extract_table_registrations(page, label: str) -> None:
        rows = await page.query_selector_all("table tbody tr")
        if not rows:
            rows = await page.query_selector_all("tr")

        logger.info(f"Found {len(rows)} rows in report {label}")
        ...
        for row in rows:
            ...
            if len(cols) >= 4:
                ...
                if "User ID" in user_id or not user_id or user_id in seen_user_ids:
                    continue
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Wait for form to load. The Trade Nation/CelleXpert login form
            # changes field names occasionally, so keep selectors defensive.
            username_input = page.locator(
                "input[name='user'], input[name='username'], input[type='text'], input:not([type])"
            ).first
            password_input = page.locator(
                "input[name='password'], input[type='password'], input[placeholder*='Password' i]"
            ).first
            submit_button = page.locator(
                "button.submit-btn, button:has-text('Sign in'), button:has-text('Login'), input[type='submit']"
            ).first

            await username_input.wait_for(timeout=120000)
            await password_input.wait_for(timeout=120000)

            await username_input.fill(TN_USERNAME)
            await password_input.fill(TN_PASSWORD)
            ...
            await extract_table_registrations(page, "initial page")
            ...
            if search_terms:
                try:
                    ...
                    for term in search_terms:
                        ...
                        logger.info(f"Searching Trade Nation report for pending name: {term}")
                        await search_input.fill(term.strip())
                        await page.wait_for_timeout(5000)
                        await extract_table_registrations(page, f"search '{term}'")

                    await search_input.fill("")
                except Exception as e:
                    logger.warning(f"Could not search Trade Nation report: {e}")
...
async def run_verification_cycle(
    ...
    if dry_run:
        logger.info("DRY RUN: matches will be logged, but no users will be verified.")
    if not send_email:
        logger.info("Email sending disabled for this run.")
    ...
    tn_registrations = await scrape_trade_nation_registrations(
        search_terms=[item.name for item in pending]
    )
    ...
    for tn_reg in tn_registrations:
        ...
        if match:
            ...
            if dry_run:
                logger.info(
                    "DRY RUN MATCH: pending_id=%s pending='%s' email='%s' tn_user_id='%s' tn_name='%s'",
                    match.id,
                    match.name,
                    match.email,
                    tn_user_id,
            ...
            if success:
                ...
                if not send_email:
                    logger.info(f"Verified without email: {match.name} <{match.email}>")
                    continue

                # Generate Telegram invite links. Bot API is tried first and
                # creates member_limit=1 links before the welcome email is sent.
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Fence Trading verification runner")
    parser.add_argument("--once", action="store_true", help="Run one verification cycle and exit.")
    parser.add_argument("--dry-run", action="store_true", help="Scrape and log matches without verifying users.")
    parser.add_argument("--no-email", action="store_true", help="Verify matched users but skip welcome emails and Telegram invite generation.")
    parser.add_argument("--interval", type=int, default=CHECK_INTERVAL, help="Seconds between checks when running continuously.")
    args = parser.parse_args()

    logger.info("Starting Verification Runner...")
    logger.info(f"Check interval: {args.interval} seconds ({args.interval // 60} minutes)")
    ...
    if TELETHON_AVAILABLE and BOT_TOKEN and not args.dry_run and not args.no_email:
        telegram_client = TelegramClient(
        ...
        logger.info("Telethon client not active. Bot API invite generation can still run.")
    ...
    try:
        while True:
            ...
            logger.info(f"Sleeping for {args.interval // 60} minutes...")
